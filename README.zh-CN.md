...
Renewlet 使用 `v0.1.0` 这类 GitHub tag 发布稳定版。每个稳定版都会发布 Docker Hub 和 GHCR 镜像，并附带 `renewlet-docker-vX.Y.Z.zip` 部署包。候选版以 prerelease 发布，只使用 `rc` Docker 标签，用于稳定版发布前验证。

日常开发进入 `dev`；`main` 只代表最新稳定版。发布和热修复分别使用 `release/vX.Y.Z` 与 `hotfix/vX.Y.Z` 分支。Pull Request 和 commit 标题应使用 Conventional Commits，例如 `feat: add notification channel` 或 `fix: prevent duplicate reminders`。

完整流程见 [发布流程](docs/release-process.zh-CN.md)。
