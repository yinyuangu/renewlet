...
- `/api/app/system/version` reports `deployment` as `docker`, `cloudflare`, or `source`, and `updateMode` as `in-app-binary`, `docker-compose`, `cloudflare-deploy`, or `source-manual`. All signed-in users may view the version state read-only; `updateSupported` only means the current signed-in user may execute the in-app binary update from the dialog, and only admins may update or restart the runtime.
